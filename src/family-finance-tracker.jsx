...
export default function FamilyFinanceTracker() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      if(saved){
        ...
        await saveData({ incomeData:SEED_DATA.incomeData, rsuData:SEED_DATA.rsuData, investmentsData:SEED_DATA.investmentsData });
      }
      setLoading(false);
    })();
  },[]);

  const persist = useCallback((iD,rD,invD)=>{
    if(saveRef.current) clearTimeout(saveRef.current);
    saveRef.current = setTimeout(()=>saveData({incomeData:iD, rsuData:rD, investmentsData:invD}), 500);
  },[]);
  ...
  const updateMonthlyIncome = (person, mi, data) => {
    ...
    setIncomeData(next);
    persist(next, rsuData, investmentsData);
  };
  ...
  const addRsuEvent = (event) => {
    ...
    setRsuData(next);
    persist(incomeData, next, investmentsData);
  };
  ...
  const updateInvestments = (fyKey, data) => {
    ...
    persist(incomeData, rsuData, next);
  };
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.bg, color:T.text, fontFamily:"'DM Sans',-apple-system,sans-serif" }}>
      ...
      <div style={{ maxWidth:"1400px", margin:"0 auto", padding:"20px" }}>
        ...
        {/* ── PLACEHOLDER TABS ── */}
        {activeTab==="expenses"&&<PlaceholderTab title="Expenses" description="Monthly budget vs actuals across categories: Rent, Groceries, Shopping, Parents, Travel, Baby, and more with colour-coded budget indicators." icon="💸"/>}
